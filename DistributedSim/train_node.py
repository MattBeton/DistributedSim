...
class TrainNode:
    ...
    def __init__(self, config: SimConfig, device: torch.device, rank: int):
        self.config = config
        self.device = device
        self.rank = rank
        ...
        if self.config.gradient_config.max_local_steps:
            self.max_steps = min(
                self.max_steps, self.config.gradient_config.max_local_steps
            )
        ...
        self.gradient_strategy = self.config.gradient_class(
            self.rank, self.model, self.config, self.logger if self.rank == 0 else None
        )
    ...
    def get_datasets(self):
        ...
        train_start = (
            (1 - self.config.dataset_config.val_proportion)
            * self.rank
            / self.config.num_nodes
            * self.config.dataset_config.dataset_proportion
        )
        train_end = (
            (1 - self.config.dataset_config.val_proportion)
            * (self.rank + 1)
        ...
        self.train_dataset = self.config.dataset_config.dataset_load_fn(
            self.config.dataset_config,
            device=self.device,
            start_pc=train_start,
            end_pc=train_end,
        )
        self.val_dataset = self.config.dataset_config.dataset_load_fn(
            self.config.dataset_config,
            device=self.device,
            start_pc=val_start,
            end_pc=val_end,
        ...
        self.train_dataloader = DataLoader(
            self.train_dataset, batch_size=self.config.minibatch_size, shuffle=True
        )

        self.val_dataloader = DataLoader(
            self.val_dataset, batch_size=self.config.minibatch_size, shuffle=True
        )
    ...
    def _save_checkpoint(self):
        ...
        if self.gradient_strategy.scheduler is not None:
            checkpoint["scheduler_state_dict"] = (
                self.gradient_strategy.scheduler.state_dict()
            )
    ...
    def _train_step(self):
        ...
        if (
            ...
            and self.local_step % self.config.checkpoint_interval == 0
        ):
    ...
    def _evaluate(self):
        ...
        if self.rank == 0:
            ...
            self.logger.log_pure(
                loss=loss_total / int(self.config.val_size / self.config.batch_size),
                name="val_local",
            )
        ...
        if self.config.num_nodes > 1:
            ...
            if self.rank == 1:
                global_loss_tensor[0] = loss_total / int(
                    self.config.val_size / self.config.batch_size
                )
    ...
    def _load_checkpoint(self):
        ...
        for step_num, f_name in checkpoint_files:
            ...
            try:
                ...
                self.gradient_strategy.optim.load_state_dict(
                    checkpoint["optimizer_state_dict"]
                )

                if (
                    "scheduler_state_dict" in checkpoint
                    and self.gradient_strategy.scheduler is not None
                ):
                    self.gradient_strategy.scheduler.load_state_dict(
                        checkpoint["scheduler_state_dict"]
                    )
